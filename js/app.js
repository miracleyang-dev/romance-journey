...
const App = (() => {
  ...
  /* nav:true = 底部导航栏, nav:false = 收入「更多」 */
  const DEFAULT_CONFIG = [
    { key:'home',       nav:true },
    { key:'dates',      nav:true },
    { key:'milestones', nav:true },
    { key:'plans',      nav:true },
    { key:'treaty',     nav:false },
    { key:'memo',       nav:false },
    { key:'travel',     nav:false },
    { key:'series',     nav:false },
  ];
  ...
  /** 底部导航栏显示的 tab 列表（nav:true 的模块 + 更多） */
  function getNavTabs() {
    const tabs = getConfig().filter(c => c.nav).map(c => ALL_MODULES.find(m => m.key === c.key)).filter(Boolean);
    tabs.push(MORE_TAB);
    return tabs;
  }

  /** 「更多」页面显示的模块列表（nav:false 的模块） */
  function getMoreModules() {
    return getConfig().filter(c => !c.nav).map(c => ALL_MODULES.find(m => m.key === c.key)).filter(Boolean);
  }
  ...
  /* ===== Render Router ===== */
  /* 每个模块 key -> 渲染函数 */
  const RENDERERS = {};  // 延迟填充（函数声明提升后可用）
  ...
  function render() {
    ...
    m.innerHTML = renderer ? renderer() : renderHome();
  }
  ...
  function renderHome() {
    ...
    if (start) {
      ...
      const na = data.couple.nameA || '我'; const nb = data.couple.nameB || '你';
  ...
  function renderMilestones() {
    ...
    const fmtNext = first._next ? `${first._next.getFullYear()}.${String(first._next.getMonth()+1).padStart(2,'0')}.${String(first._next.getDate()).padStart(2,'0')}` : '';

    let html = `<div class="ms-hero"><div class="ms-hero__label">距离下一个节点</div><div class="ms-hero__title">${esc(first.title)}</div><div class="ms-hero__days">${first._days === 0 ? '今天！' : first._days + ' 天'}</div><div class="ms-hero__date">${first.isLunar ? '农历' : '公历'} ${first.month}月${first.day}日 · 下次 ${fmtNext}</div><div class="ms-hero__actions"><button onclick="App.editMilestone(${first.id})">编辑</button><button class="del" onclick="App.del('milestones',${first.id})">删除</button></div></div>`;
    html += `<div class="ms-list">`;
    rest.forEach(i => {
      const nxt = i._next ? `${i._next.getFullYear()}.${String(i._next.getMonth()+1).padStart(2,'0')}.${String(i._next.getDate()).padStart(2,'0')}` : '';
      html += `<div class="ms-card"><div class="ms-card__left"><div class="ms-card__title">${esc(i.title)}</div><div class="ms-card__info">${i.isLunar?'农历':'公历'} ${i.month}月${i.day}日</div></div><div class="ms-card__right"><div class="ms-card__next">下次 ${nxt}</div><div class="ms-card__actions"><button onclick="App.editMilestone(${i.id})">编辑</button><button onclick="App.del('milestones',${i.id})" style="color:#c0392b">删除</button></div></div></div>`;
    });
  ...
  function saveMilestone(id) { saveItem('milestones',id,{title:v('f_title'),month:parseInt(v('f_month')),day:parseInt(v('f_day')),isLunar:v('f_lunar')==='1'}); closeModal(); render(); }

  /* ========================= DATES ========================= */
  /** 格式化日期显示：统一紧凑格式 */
  function fmtDateRange(item) {
    if (item.dateEnd && item.dateEnd !== item.date) {
      // 同年省略重复年份：2024.06.08 ~ 06.10
      ...
      return `<div class="date-card">
        ...
        <div class="date-card__inner">
          <div class="date-card__top">
            <span class="date-card__tag ${isRange ? 'range' : ''}">${fmtDateRange(i)}</span>
          </div>
          <div class="date-card__title">${esc(i.event)}</div>
          ${i.note ? `<div class="date-card__note">${esc(i.note)}</div>` : ''}
          <div class="card__actions">
            <button onclick="App.editDate(${i.id})">编辑</button>
            <button class="del" onclick="App.del('dates',${i.id})">删除</button>
          </div>
        </div>
      </div>`;
    }).join('') + addBtn('添加约会','editDate()');
  }
  function editDate(id) {
    ...
    showModal(id?'编辑约会':'添加约会', `
      <label>时间类型</label>
      <select id="f_datetype" onchange="App.toggleDateRange('date')">
  ...
  function saveDate(id) {
    ...
    const dateEnd = isRange ? v('f_dateEnd') : '';
    saveItem('dates',id,{date:v('f_date'),dateEnd:dateEnd,event:v('f_event'),note:v('f_note')});
    closeModal(); render();
  }
  ...
  function renderPlans() {
    ...
    const slips = items.map((p,idx)=>{
      const c='note-colors-'+(idx%6);
      const t = p.done ? `<span class="done-mark">${esc(p.title)}</span>` : esc(p.title);
      return `<div class="note-slip ${c}" onclick="App.viewPlan(${p.id})">${t}</div>`;
    }).join('');
  ...
  function savePlan(id) {
    ...
    if(id){const old=(data.plans||[]).find(i=>i.id===id);obj.done=old?old.done:false;}else obj.done=false;
    saveItem('plans',id,obj); closeModal(); render();
  }
  function togglePlan(id){const p=(data.plans||[]).find(i=>i.id===id);if(p){p.done=!p.done;persist();closeModal();render();}}
  ...
  function renderTreaty() {
    ...
    let list = `<div class="treaty-list ${modeClass}" id="treatyList">` + items.map((t, idx) => {
      ...
      return `<div class="treaty-item" draggable="${draggable}" data-tidx="${idx}"
        ondragstart="App.treatyDragStart(event,${idx})"
        ondragover="App.treatyDragOver(event,${idx})"
        ondragleave="App.treatyDragLeave(event)"
        ondrop="App.treatyDrop(event,${idx})"
        ondragend="App.treatyDragEnd(event)">
        ...
        ${childrenHtml}
        <div class="card__actions treaty-edit-only">
          <button onclick="App.editSubTreaty(${t.id},0)">+子条约</button>
          <button onclick="App.editTreaty(${t.id})">编辑</button>
          <button class="del" onclick="App.del('treaties',${t.id})">删除</button>
        </div>
      </div>`;
    }).join('') + `</div>`;

    const addBtnHtml = _treatyEditMode ? addBtn('添加条款', 'editTreaty()') : '';
    return header + list + addBtnHtml;
  }

  function toggleTreatyMode() {
    _treatyEditMode = !_treatyEditMode;
    render();
  }

  /* Treaty drag-and-drop */
  function treatyDragStart(e, idx) {
    _treatyDragIdx = idx;
    e.currentTarget.classList.add('dragging');
    e.dataTransfer.effectAllowed = 'move';
  }
  function treatyDragOver(e, idx) {
    e.preventDefault();
    e.dataTransfer.dropEffect = 'move';
    const el = e.currentTarget;
    if (idx !== _treatyDragIdx) el.classList.add('drag-over');
  }
  ...
  function treatyDragEnd(e) {
    _treatyDragIdx = null;
    e.currentTarget.classList.remove('dragging');
    document.querySelectorAll('.treaty-item').forEach(el => el.classList.remove('drag-over'));
  }

  function editTreaty(id) {
    const item = id ? (data.treaties||[]).find(i=>i.id===id) : { content:'' };
    const examples = '参考：不许和异性单独吃饭 / 吵架不过夜 / 每周至少约会一次 / 重要节日必须一起过 / 手机可以互相看';
    showModal(id ? '编辑条款' : '添加条款', `
      <label>条款内容</label>
      <textarea id="f_content" style="min-height:100px" placeholder="写下你们的约定">${esc(item.content)}</textarea>
      <div class="hint-text">${examples}</div>
  ...
  function editSubTreaty(parentId, childId) {
    const parent = (data.treaties||[]).find(i => i.id === parentId);
    if (!parent) return;
  ...
  function saveSubTreaty(parentId, childId) {
    const content = document.getElementById('f_sub_content')?.value || '';
    if (!content.trim()) return;
    const parent = (data.treaties||[]).find(i => i.id === parentId);
    if (!parent) return;
  ...
  /* ========================= MORE ========================= */
  function renderMore() {
    // 「更多」页展示所有 nav:false 的模块
    const moreModules = getMoreModules();
    if (!moreModules.length) return empty('所有功能已在底部导航栏显示');
    return `<div class="more-grid">${moreModules.map(mod =>
  ...
  function saveMemo(id) { saveItem('memos',id,{title:v('f_title'),content:document.getElementById('f_content')?.value||''}); closeModal(); render(); }

  /* ========================= TRAVEL ========================= */
  function renderTravel() {
    const items=(data.travels||[]).slice().sort((a,b)=>b.date.localeCompare(a.date));
    const count=items.length;
    let summary=count?`<div style="text-align:center;padding:.8rem 0;font-size:.9rem;color:var(--text2)">已打卡 <strong style="color:var(--accent);font-size:1.2rem">${count}</strong> 个地方</div>`:'';
  ...
  function editTravel(id) {
    ...
    showModal(id?'编辑足迹':'添加足迹',`
      ...
      <label>时间类型</label>
      <select id="f_datetype" onchange="App.toggleDateRange('travel')">
        <option value="point" ${!isRange?'selected':''}>单日</option>
        <option value="range" ${isRange?'selected':''}>时间段</option>
      </select>
      <label>日期</label><input type="date" id="f_date" value="${item.date}">
  ...
  function saveTravel(id) {
    const isRange = v('f_datetype') === 'range';
    const dateEnd = isRange ? v('f_dateEnd') : '';
    saveItem('travels',id,{place:v('f_place'),date:v('f_date'),dateEnd:dateEnd,note:v('f_note')});
    closeModal(); render();
  }
  ...
  function saveSeriesTitle(id) {
    if(id){const s=(data.series||[]).find(i=>i.id===id);if(s){s.title=v('f_title');s.note=v('f_note');}}
    else{if(!data.series)data.series=[];data.series.push({id:Store.nextId(data.series),title:v('f_title'),note:v('f_note'),items:[]});}
    persist(); closeModal(); render();
  }
  ...
  function saveSeriesItem(seriesId, itemId) {
    const s=(data.series||[]).find(i=>i.id===seriesId); if(!s)return;
    if(!s.items)s.items=[];
    const fields={title:v('f_title'),date:v('f_date'),content:document.getElementById('f_content')?.value||''};
  ...
  function editNavConfig() {
    const cfg = getConfig();
    window._navEditCfg = JSON.parse(JSON.stringify(cfg));
    renderNavEditor();
  }

  function renderNavEditor() {
    const cfg = window._navEditCfg;
    let listHtml = cfg.map((item, idx) => {
      const def = ALL_MODULES.find(t => t.key === item.key);
      const label = def ? def.label : item.key;
      const isHome = item.key === 'home'; // 首页不可隐藏
    ...
    const visibleCount = cfg.filter(c => c.nav).length;

    showModal('底部导航布局', `
      <div style="font-size:.8rem;color:var(--text2);margin-bottom:.6rem">开关控制「底部导航/更多」，箭头调整排序。当前导航栏 ${visibleCount} 项（建议 3-5 项）。</div>
      <div class="nav-editor" style="border-radius:var(--radius);overflow:hidden;border:1px solid var(--border)">${listHtml}</div>
      <div class="modal__footer">
        <button class="btn-secondary" onclick="App.resetNavConfig()">恢复默认</button>
        <button class="btn-primary" onclick="App.saveNavConfig()">保存</button>
      </div>
    `);
  }
  ...
  function navToggle(idx) {
    const cfg = window._navEditCfg;
    if (cfg[idx].key === 'home') return;
    cfg[idx].nav = !cfg[idx].nav;
    renderNavEditor();
  }
  ...
  function navMove(idx, dir) {
    ...
    [cfg[idx], cfg[target]] = [cfg[target], cfg[idx]];
    renderNavEditor();
  }

  function saveNavConfig() {
    data.navConfig = window._navEditCfg;
    delete window._navEditCfg;
    persist(); closeModal(); renderNav(); render();
  }

  function resetNavConfig() {
    window._navEditCfg = DEFAULT_CONFIG.map(n => ({...n}));
    renderNavEditor();
  }
  ...
  function saveItem(key, id, fields) {
    ...
    if (id) { const idx = data[key].findIndex(i => i.id === id); if (idx >= 0) Object.assign(data[key][idx], fields); }
    else { fields.id = Store.nextId(data[key]); data[key].push(fields); }
    persist();
  }
  function del(key, id) {
    if (!confirm('确定删除？')) return;
  ...
  function mergeData(imported) {
    ...
    // 数组类字段：全部追加，然后按顺序重编 id
    const arrKeys = ['milestones','dates','plans','memos','travels','treaties','photos'];
    ...
    if (Array.isArray(imported.series)) {
      ...
      data.series.forEach((s, idx) => {
        ...
        if (Array.isArray(s.items)) {
          s.items.forEach((item, i) => { item.id = i + 1; });
        }
      });
    }
  }
  function clearData() {
    ...
    data = JSON.parse(JSON.stringify(Store.defaultData)); persist(); renderNav(); render();
  }
